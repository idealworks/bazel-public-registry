...
def _default_compiler_flags(ctx, is_cpp = False):
...
def _impl(ctx):
    default_compiler_flags = _default_compiler_flags(ctx, is_cpp = False)
    default_cpp_compiler_flags = _default_compiler_flags(ctx, is_cpp = True)
    ...
    c_compiler_flags = feature(
        ...
        flag_sets = [
            flag_set(
                ...
                flag_groups = [
                    flag_group(flags = ["-I" + include.path for include in ctx.files.c_include_path]),
    ...
    cpp_compiler_flags = feature(
        ...
        flag_sets = [
            flag_set(
                ...
                flag_groups = [
                    flag_group(flags = ["-I" + include.path for include in ctx.files.cpp_include_path]),
                    flag_group(flags = ctx.attr.copts + default_compiler_flags),
                ],
            ),
        ],
...
cc_arm_gnu_toolchain_config = rule(
    ...
    attrs = {
        ...
        "c_include_path": attr.label_list(default = [], allow_files = True),
        "cpp_include_path": attr.label_list(default = [], allow_files = True),
