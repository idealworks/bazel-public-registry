...
def architectue_based_flag(value):
    ...
    return select({
        "@pcl//:__pcl_linux-aarch64": _PCL_AARCH64_COMPILER_CONFIG[value],
        "//conditions:default": _PCL_DEFAULT_COMPILER_CONFIG[value],
    })
...
def _gen_pcl_config_impl(ctx):
    ...
    pcl_index_signed_str = "true" if ctx.attr.pcl_index_signed[BuildSettingInfo].value else "false"
    substitutions = {
        ...
        "${PCL_INDEX_SIZE}": "{}".format(ctx.attr.pcl_index_size[BuildSettingInfo].value),
        "${PCL_INDEX_SIGNED}": pcl_index_signed_str,
    ...
    return [
        ...
        DefaultInfo(files = depset([ctx.outputs.pcl_config_hdr])),
    ]
...
gen_pcl_config = rule(
    ...
    attrs = {
        ...
        "pcl_index_signed": attr.label(
            providers = [BuildSettingInfo],
            default = Label("//:pcl_index_signed"),
        ),
        "pcl_index_size": attr.label(
            providers = [BuildSettingInfo],
            default = Label("//:pcl_index_size"),
        ),
